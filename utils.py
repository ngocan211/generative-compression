...
class Utils(object):
    ...
    @staticmethod
    def run_diagnostics(model, config, directories, sess, saver, train_handle, start_time, epoch, name, G_loss_best,
                        D_loss_best):
        t0 = time.time()
        improved = ''
        sess.run(tf.local_variables_initializer())
        try:
            G_loss, D_loss, summary = sess.run([model.G_loss, model.D_loss, model.merge_op],
                                               feed_dict={model.training_phase: False, model.handle: train_handle})
        ...
        if G_loss < G_loss_best and D_loss < D_loss_best:
            ...
            if epoch > 5:
                save_path = saver.save(sess,
                                       os.path.join(directories.checkpoints_best,
                                                    '{}_epoch{}.ckpt'.format(name, epoch)),
                                       global_step=epoch)
                print('Graph saved to file: {}'.format(save_path))

        if epoch % 5 == 0 and epoch > 5:
            save_path = saver.save(sess, os.path.join(directories.checkpoints, '{}_epoch{}.ckpt'.format(name, epoch)),
                                   global_step=epoch)
            print('Graph saved to file: {}'.format(save_path))

        print(
            'Epoch {} | Generator Loss: {:.3f} | Discriminator Loss: {:.3f} | Rate: {} examples/s ({:.2f} s) {}'.format(
                epoch, G_loss, D_loss, int(config.batch_size / (time.time() - t0)), time.time() - start_time, improved))
    ...
    def single_plot(epoch, global_step, sess, model, handle, name, config, single_compress=False):
        ...
        r, g = sess.run([model.example, model.reconstruction],
                        feed_dict={model.training_phase: True, model.handle: handle})
        ...
        if single_compress:
            ...
            f.savefig("{}/gan_compression_{}_epoch{}_step{}_{}_comparison.pdf".format(directories.samples, name, epoch,
                                                                                      global_step, imtype),
                      format='pdf', dpi=720, bbox_inches='tight', pad_inches=0)
        plt.gcf().clear()
        plt.close(f)
